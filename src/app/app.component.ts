...
export class AppComponent implements OnInit {
    ...
    ngOnInit() {
        for (let card of this.deckObject['mainboard']) {
            for (let i = 0; i < card['quantity']; i++) {
                let newCard = Object.assign({}, card['card']);
                newCard['goldfishId'] = Math.random().toString(36).substring(2);
                this._sharedService.libraryZone.push(newCard);
            }
        }

        this._configuration.shuffle(this._sharedService.libraryZone);
    ...
    public performAction(event): any {
        switch (event) {
           ...
           case 'shuffle_hand': {
             this._configuration.shuffle(this._sharedService.handZone);
             break;
           }
           case 'mulligan': {
               this._sharedService.libraryZone = this._sharedService.libraryZone.concat(this._sharedService.handZone);
               this._sharedService.handZone = [];
               this._configuration.shuffle(this._sharedService.libraryZone);
    ...
    public drawCard(): any {
        const newCard = this._sharedService.libraryZone.pop();
        this._sharedService.handZone.push(newCard);
    }
}
